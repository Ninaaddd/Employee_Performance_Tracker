...
def show_dashboard():
    ...
    try:
        ...
        with col4:
            ...
            if employees:
                for emp in employees:
                    emp_projects = get_projects_for_employee(emp['employee_id'])
                    if emp_projects:
        ...
        if employees:
            ...
            st.dataframe(
                df_sorted[['first_name', 'last_name', 'email', 'department', 'hire_date']],
...
def show_employee_management():
    ...
    st.markdown('<p class="main-header">👥 Employee Management</p>', unsafe_allow_html=True)
    
    tabs = st.tabs(["📋 View Employees", "➕ Add Employee", "✏️ Edit Employee", "🔍 Search"])
                ...
                if selected:
                    ...
                    if employee:
                        with st.form("edit_employee_form"):
                            ...
                            with col1:
                                new_first = st.text_input("First Name", value=employee['first_name'])
                                new_last = st.text_input("Last Name", value=employee['last_name'])
                                new_email = st.text_input("Email", value=employee['email'])
                            
                            with col2:
                                new_hire_date = st.date_input(
                                    "Hire Date",
                                    value=datetime.strptime(employee['hire_date'], '%Y-%m-%d').date()
                                )
                                new_dept = st.selectbox(
                                    "Department",
                                    ["Engineering", "Sales", "Marketing", "HR", "Finance", "Operations", "Other"],
                                    index=["Engineering", "Sales", "Marketing", "HR", "Finance", "Operations", "Other"].index(employee['department']) if employee['department'] in ["Engineering", "Sales", "Marketing", "HR", "Finance", "Operations", "Other"] else 0
                                )
                            ...
                            with col_btn2:
                                delete_btn = st.form_submit_button("🗑️ Delete Employee", use_container_width=True, type="secondary")
                            ...
                            if delete_btn:
                                try:
                                    ...
                                    if success:
                                        ...
                                        st.error("Cannot delete: Employee has assignments")
                                except Exception as e:
...
def show_project_management():
    ...
    tabs = st.tabs(["📋 View Projects", "➕ Add Project", "🔗 Assign Employees", "👥 Project Teams"])
    ...
    with tabs[0]:
        ...
        try:
            ...
            if projects:
                ...
                df_filtered = df[df['status'].isin(status_filter)].sort_values(sort_by)
        ...
        try:
            ...
            if employees and projects:
                with st.form("assign_form"):
                    ...
                    role = st.text_input("Role/Position *", placeholder="e.g., Lead Developer, Designer")
                        ...
                        if not role:
                            ...
                            try:
                                ...
                                if success:
                                    ...
                                    st.error("Assignment failed. May already exist.")
                            except Exception as e:
                                st.error(f"Error: {str(e)}")
            else:
    ...
    with tabs[3]:
        ...
        try:
            ...
            if projects:
                proj_options = {p['project_name']: p['project_id'] for p in projects}
                selected = st.selectbox("Select Project", list(proj_options.keys()))
...
def show_performance_reviews():
    ...
    st.markdown('<p class="main-header">⭐ Performance Reviews</p>', unsafe_allow_html=True)
                    ...
                    if submitted:
                        if not all([selected_emp, reviewer_name]):
                            ...
                            try:
                                ...
                                improvements_list = [i.strip() for i in improvements.split('\n') if i.strip()]
                ...
                if selected:
                    ...
                    if reviews:
                        ...
                        for i, review in enumerate(reviews, 1):
                            rating = safe_convert_rating(review.get('overall_rating'))
                            rating_stars = "⭐" * int(rating)
                            with st.expander(f"Review {i} - {review.get('review_date','N/A')} {rating_stars} ({rating:.1f})"):
                                col1, col2 = st.columns(2)
                                
                                with col1:
                                    st.markdown(f"**Reviewer:** {review.get('reviewer_name', 'Unknown')}")
                                    st.markdown(f"**Date:** {review.get('review_date', 'N/A')}")
                                    st.markdown(f"**Rating:** {rating:.1f}/5.0 {rating_stars}")
    ...
    with tabs[2]:
        ...
        try:
            ...
            if employees:
                ...
                for emp in employees:
                    reviews = get_performance_reviews_for_employee(emp['employee_id'])
                    if reviews:
                        for review in reviews:
                            rating = safe_float_conversion(review.get('overall_rating', 0))
                            ratings_data.append({
                ...
                if ratings_data:
                    ...
                    with col1:
                        ...
                        avg_by_dept = df.groupby('Department')['Rating'].mean().reset_index()
                        fig = px.bar(
                            avg_by_dept,
                            x='Department',
                            y='Rating',
                    ...
                    top_performers = df.groupby('Employee')['Rating'].mean().sort_values(ascending=False).head(5)
...
def show_reports():
    ...
    tabs = st.tabs(["📋 Employee-Project Report", "⭐ Performance Summary", "📈 Custom Reports"])
        ...
        try:
            ...
            if report_data:
                ...
                if 'employee_name' in df.columns and 'project_name' in df.columns:
                    ...
                    assignments_per_emp = df.groupby('employee_name').size().reset_index(name='count')
                    fig = px.bar(
                ...
                if selected:
                    ...
                    if employee:
                        ...
                        with col3:
                            ...
                            st.markdown(f"**Tenure:** {tenure_years:.1f} years")
                        ...
                        if reviews:
                            ...
                            with col3:
                                latest_rating = reviews[0].get('overall_rating', 0) if reviews else 0
                                delta = latest_rating - avg_rating if len(reviews) > 1 else 0
                                st.metric("⭐ Latest Rating", f"{latest_rating:.1f}", delta=f"{delta:+.1f}")
                            ...
                            if all_strengths:
                                ...
                                for strength, count in strength_counts.most_common(5):
                                    st.markdown(f"- **{strength}** (mentioned {count} time{'s' if count > 1 else ''})")
                            ...
                            if all_improvements:
                                ...
                                for area, count in improvement_counts.most_common(3):
                                    st.markdown(f"- {area} (mentioned {count} time{'s' if count > 1 else ''})")
                            ...
                            st.info("No performance reviews available for this employee")
            else:
                st.info("No employees available")
        ...
        if st.button("Generate Report", type="primary"):
            try:
                if report_type == "Employees by Department":
                    ...
                    if employees:
                        df = pd.DataFrame(employees)
                        summary = df.groupby('department').size().reset_index(name='count')
                        
                        st.dataframe(summary, use_container_width=True)
                        
                        fig = px.pie(summary, values='count', names='department', title="Employee Distribution")
                        st.plotly_chart(fig, use_container_width=True)
                    ...
                    if projects:
                        ...
                        fig = px.bar(summary, x='status', y='count', title="Projects by Status")
                        st.plotly_chart(fig, use_container_width=True)
                    ...
                    for emp in employees:
                        projects = get_projects_for_employee(emp['employee_id'])
                        if not projects or len(projects) == 0:
                            unassigned.append(emp)
                    
                    if unassigned:
                        st.warning(f"Found {len(unassigned)} unassigned employees")
...
def show_settings():
    ...
    with tabs[0]:
        ...
        try:
            ...
            with col1:
                ...
                st.metric("Total Employees", len(employees) if employees else 0)
            ...
            with col2:
                ...
                try:
                    ...
                    if employees:
                        test_reviews = get_performance_reviews_for_employee(employees[0]['employee_id'])
    ...
    with tabs[1]:
        ...
        with col1:
            ...
            if st.button("📥 Backup Database", use_container_width=True):
                try:
                    ...
                    shutil.copy2("company.db", f"company_backup_{timestamp}.db")
                    st.success(f"✅ Backup created: company_backup_{timestamp}.db")
                except Exception as e:
        ...
        with st.expander("Clear Sample Data"):
            st.error("This will delete all sample/test data. This action cannot be undone!")
            
            confirm = st.text_input("Type 'DELETE' to confirm")
            
            if st.button("🗑️ Delete Sample Data", type="secondary"):
                if confirm == "DELETE":
                    st.warning("This feature would delete sample data (implement with care)")
